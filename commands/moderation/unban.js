...
module.exports.run = async(client, message, args) => {
    ...
    if (!message.member.hasPermission('BAN_MEMBERS')) return message.channel.send(embed.setDescription(`Sorry! You\'re missing the permission \`BAN_MEMBERS\` to use this command.`).setFooter(`${message.guild.name} | Made By Fuel#2649`, message.guild.iconURL({ dynamic: true })))
    ...
    await message.guild.fetchBan(args[0]).then(() => {
        ...
        return message.channel.send(embed.setDescription('Sorry but.. that user is not banned in this guild.').setFooter(`${message.guild.name} | Made By Fuel#2649`, message.guild.iconURL({ dynamic: true })))
    })
}