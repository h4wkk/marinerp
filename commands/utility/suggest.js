...
module.exports.run = async(client, message, args) => {
    ...
    if (!channel) return message.channel.send(new client.embed().setDescription('A suggestions channel hasnt been setup for this server!').setFooter(`${message.guild.name} | Made By Fuel#2649`, message.guild.iconURL({ dynamic: true })))
    if (!args[0]) return message.channel.send(new client.embed().setDescription(`Please provide me a suggestion!`).setFooter(`${message.guild.name} | Made By Fuel#2649`, message.guild.iconURL({ dynamic: true })));
