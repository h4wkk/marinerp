...
module.exports.run = async(client, message, args) => {
    ...
    if (!description.length) embed2.setFooter(`${message.guild.name} | Made By Fuel#2649`, message.guild.iconURL({ dynamic: true }))
    ...
    if (description.length) chan.send(new client.embed().setDescription('This application was split due to discord character limit.').setFooter(`${message.guild.name} | Made By Fuel#2649`, message.guild.iconURL({ dynamic: true })))
}