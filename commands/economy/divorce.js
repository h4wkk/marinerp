...
module.exports.run = (client, message) => {
    ...
    const embed = new client.embed().setDescription(`Are you sure you want to divorce from ${arr[1]} (\`${arr[0]}\`)?`).setFooter(`${message.guild.name} | Made By Fuel#2649`, message.guild.iconURL({ dynamic: true }))
    ...
    message.channel.send(embed).then(async r2 => {
        ...
        r2.awaitReactions((r, u) => ['✅', '❌'].includes(r.emoji.name) && u.id === message.author.id, { max: 1, time: 30000 }).then(async r => {
            if (r.first().emoji.name === '✅') {
                r2.edit(embed.setDescription(`${embed.description}\n:broken_heart: You decided to end things!`).setFooter(`${message.guild.name} | Made By Fuel#2649`, message.guild.iconURL({ dynamic: true })))
                ...
                r2.edit(embed.setDescription(`${embed.description}\n:gift_heart: You decided to stay together!`).setFooter(`${message.guild.name} | Made By Fuel#2649`, message.guild.iconURL({ dynamic: true })))
        }).catch(() => {})
    })
}