...
module.exports.run = async(client, message, args) => {
    ...
    if (!vcSettings.Allowed_Roles.some(s => message.member.roles.cache.has(s))) return message.channel.send(new client.embed().setDescription('Sorry, but you cant create a temp vc!').setFooter(`${message.guild.name} | Made By Fuel#2649`, message.guild.iconURL({ dynamic: true })))
    if (!args[0]) return message.channel.send(new client.embed().setDescription('You need to enter what you want the name of the vc to be!').setFooter(`${message.guild.name} | Made By Fuel#2649`, message.guild.iconURL({ dynamic: true })))
    ...
    await message.channel.send(new client.embed().setDescription(`Successfully created a temporary vc named ${args.join(' ')}`).setFooter(`${message.guild.name} | Made By Fuel#2649`, message.guild.iconURL({ dynamic: true })))
